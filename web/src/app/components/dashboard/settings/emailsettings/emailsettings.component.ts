...
export class EmailsettingsComponent implements OnInit, OnDestroy {
    ...
    inboundEdit() {
         this._subs = this.companyService.getSettings(this.company._id, 'imap').subscribe(result => {
            this.inboundSettings = result['success']['data'];
        });
    }

    outboundEdit() {
         this._subs = this.companyService.getSettings(this.company._id, 'smtp').subscribe(result => {
            this.outboundSettings = result['success']['data'];
        });
    }

    templateEdit() {
         this._subs = this.companyService.getSettings(this.company._id, 'template').subscribe(result => {
