...
export class UsersComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this._subs = this.accountService.getRoles().subscribe(result => {
    ...
    onFilterChange(filter: any) {
        ...
        this._subs = this.accountService.getAllUsers(this.filter).subscribe(result => {
            if (result['success']) {
    ...
    populateForm(details) {
        if (details) {
            const roleId = details.roles.length ? details.roles[0]._id : null;
    ...
    create(adminFormDetail) {
        if (!this.adminDetails.valid) {
            ...
            this._subs = this.accountService.createUser(adminFormDetail).subscribe(result => {
                if (result['success']['data']) {
    ...
    getUser(id) {
        this._subs = this.accountService.getUserById(id).subscribe(result => {
            if (result['success']) {
                this.userId = id;
    ...
    editUser(adminFormDetail, id: any) {
        if (!this.adminDetails.valid) {
            ...
            this._subs = this.accountService.update(adminFormDetail, id).subscribe(result => {
                if (result['success']['data']) {
                    this.adminDetails.reset();
                    const close = document.getElementById('close');
                    close.click();
    ...
    confirm(): void {
        if (this.userId) {
            this._subs = this.accountService.delete(this.userId).subscribe(result => {
                if (result['success']['data']) {
                } else {
    ...
    getRoleName(roleId) {
        const role = this.roles.filter(r => r.id == roleId);
        if (role.length > 0) {
            return role[0].name;
        }
        return 'Undefined';
