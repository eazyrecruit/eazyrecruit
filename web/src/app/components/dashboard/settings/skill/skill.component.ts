...
export class SkillComponent implements OnInit, OnDestroy {
    ...
    createSkills(skillsForm: any) {
         this._subs = this.skillsService.saveSkills(skillsForm).subscribe(result => {
            if (result['success']) {
                this.onFilterChange(this.filter);
    ...
    showSkillList(limit: any, offset: any) {
         this._subs = this.skillsService.getSkills(this.filter).subscribe(result => {
            if (result['success']) {
    ...
    editSkill(skillsForm: any, id: any) {
        skillsForm.id = id;
         this._subs = this.skillsService.editSkill(skillsForm, this.skillId).subscribe(result => {
            if (result['success']) {
                this.skillsDetail.reset();
                const close = document.getElementById('close');
                close.click();
    ...
    confirm(): void {
        if (this.skillId) {
             this._subs = this.skillsService.deleteSkill(this.skillId).subscribe(skillResult => {
